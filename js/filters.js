...
// Internal mutations use clear-in-place (delete per key) rather than reassignment.
...
/** Clear activeFilters in-place, preserving the exported object reference. */
...
function _buildPanels() {
    ...
    _defs.forEach((def, idx) => {
      ...
      <div class="fg-combo" id="fgc-${idx}">
        <div class="fg-combo-input dd-trigger" id="fgci-${idx}">
          <input id="fgi-${idx}" class="fg-search" type="text"
                 autocomplete="off" spellcheck="false"
                 placeholder="${t('dropdown.search', 0)}">
          <span class="fg-combo-arrow dd-chevron"><svg width="6" height="6" aria-hidden="true"><use href="#icon-panel-arrow"></use></svg></span>
        </div>
        <div class="fg-dropdown dd-panel is-hidden" id="fgd-${idx}">
...
function _openDropdown(idx) {
    ...
    _defs.forEach((_, i) => { if (i !== idx) _closeDropdown(i); });
    // Close language dropdown if open
    document.getElementById('lang-dropdown')?.classList.remove('open');
    const dd = document.getElementById(`fgd-${idx}`);
    if (dd) dd.classList.remove('is-hidden');
}
